...
function datet($date){
	global $bdd;
	// $var = ucfirst(strftime('%A, le  %d',strtotime($date)));
	// $var .= ucfirst(strftime('%B %Y',strtotime($date)));	
	$var = ucfirst(strftime('%A, le  %d',strtotime($date)));
	$var .= ucfirst(strftime('%B %Y a %T',strtotime($date)));	
...
function get_video($titre){
	global $bdd;
	$name = $bdd->prepare("SELECT * FROM article WHERE titre= ?");
	$name->execute(array($titre));
	$name = $name->fetch();
	$id = $name['id'];
	return $id;
}
...
function get_id($id){
	global $bdd;
	$name = $bdd->prepare("SELECT * FROM article WHERE id= ?");
	$name->execute(array($titre));
	$name = $name->fetch();
	$id = $name['id'];
	return $id;
...
function pagination($reponsesParpages = 5,$reponsesTotalesReq){
		// $reponsesTotalesReq = $bdd->query($req);
